...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        # Shared QKV projection with separate heads
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        v_dim = self.n_kv_head * self.head_dim
        q = qkv[:, :, :q_dim].view(B, T, self.n_head, self.head_dim)
        k = qkv[:, :, q_dim:q_dim+k_dim].view(B, T, self.n_kv_head, self.head_dim)
        v = qkv[:, :, q_dim+k_dim:q_dim+k_dim+v_dim].view(B, T, self.n_kv_head, self.head_dim)
