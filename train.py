...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        assert len(list(self.parameters())) == (len(matrix_params) + len(attn_proj_params) + len(embedding_params) +
        ...
        for shape in sorted({p.shape for p in attn_proj_params}):
            group_params = [p for p in attn_proj_params if p.shape == shape]
            param_groups.append(dict(
                kind='muon', params=group_params, lr=matrix_lr,
                momentum=0.95, ns_steps=5, beta2=0.95, weight_decay=0.001,
            ))
