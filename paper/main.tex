...
\midrule
Claude Sonnet 4     & Anthropic     & None       & Adaptive (0--0.5)  & 180s  & \$0.16 \\
Claude Sonnet 4.6   & Anthropic     & None       & Adaptive (0--0.5)  & 180s  & \$0.06 \\
Claude Opus 4.6     & Anthropic     & 32K tokens & 1.0 (API req.)     & 900s  & \$0.66 \\
\bottomrule
\end{tabular}
\end{table}

GPT-4.1 was evaluated in Phase 2 only (Section~\ref{sec:phase2}).

Opus 4.6 uses temperature 1.0, which is required by Anthropic's extended thinking API (not a tunable parameter). The 900-second timeout accommodates the longer response times when the model uses its full 32K thinking budget on complex prompts with long experiment histories. The higher cost per experiment (\$0.66 vs.\ \$0.06) reflects the extended thinking token usage, which is billed as output tokens. All per-experiment costs are verified from actual Anthropic billing data, not token-math estimates.
...
In Phase 1, models were run as a relay: Sonnet 4.6 optimized the training code from an unmodified baseline, then Opus 4.6 continued from Sonnet 4.6's best result. (Sonnet 4 ran separately on a different baseline as an earlier pilot.) This sequential design means each model faced a progressively harder optimization landscape -- Opus 4.6 had to find improvements that Sonnet 4.6 could not, not compete on the same starting code. Phase 1 should therefore be read as a relay narrative (Figure~\ref{fig:relay}), not a head-to-head competition. Phase 2 (Section~\ref{sec:phase2}) provides the controlled comparison.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/relay_improvement.png}
\caption{Phase 1 optimization relay. Sonnet 4.6 made 21 improvements from an unoptimized baseline (val\_bpb 1.245 $\to$ 0.956), then Opus 4.6 with extended thinking continued from that optimized code and found 8 further improvements (0.956 $\to$ 0.902). Key discoveries are annotated. The x-axis shows cumulative kept improvements, not total experiments.}
\label{fig:relay}
\end{figure}
...
As shown in Figure~\ref{fig:relay}, Phase 1 is best understood as a relay: Sonnet 4.6 handled the initial optimization (finding big wins like batch size halving and warmdown tuning), then Opus 4.6 took over and found subtler improvements requiring multi-step reasoning. Comparing their keep rates or improvement percentages directly is misleading because they operated on fundamentally different difficulty levels. Phase 2 addresses this with a controlled identical-baseline design.
...
\caption{Keep rate (non-crash experiments that improved val\_bpb) and crash rate by model. Sonnet 4.6 achieved the highest keep rate and lowest crash rate among the three Phase 1 models.}
...
\begin{table}[H]
\centering
\caption{Cost-effectiveness analysis. API costs are from actual Anthropic billing data (March 2026). GPU cost assumes \$0.50/hr equivalent for electricity and amortization.}
\label{tab:cost}
\begin{tabular}{lrrr}
\toprule
\textbf{Metric} & \textbf{Sonnet 4} & \textbf{Sonnet 4.6} & \textbf{Opus 4.6} \\
\midrule
API cost (actual)    & \$22.92 & \$6.28  & \$73.09  \\
GPU time (hrs)       & 12.25   & 8.67    & 9.25     \\
GPU cost (est.)      & \$6.13  & \$4.33  & \$4.63   \\
Total cost           & \$29.05 & \$10.61 & \$77.72  \\
Improvement (\%)     & 1.25    & 23.24   & 5.64     \\
\textbf{Cost / 1\% impr.} & \textbf{\$23.24} & \textbf{\$0.46}  & \textbf{\$13.78}  \\
\bottomrule
\end{tabular}
\end{table}
...
% =============================================================================
\section{Phase 2: Controlled Head-to-Head Comparison}
\label{sec:phase2}
% =============================================================================
...
Table~\ref{tab:phase2_runs} summarizes the run schedule. GPT-4.1 runs used Azure credits at zero marginal cost. Sonnet 4.6 runs used the Anthropic API. Opus and Sonnet 4 runs are deferred due to budget constraints.

\begin{table}[H]
\centering
\caption{Phase 2 controlled comparison: run schedule and status.}
\label{tab:phase2_runs}
\begin{tabular}{llllr}
\toprule
\textbf{Run} & \textbf{Model} & \textbf{API} & \textbf{Status} & \textbf{Actual Cost} \\
\midrule
1--3  & GPT-4.1           & Azure OpenAI & Complete & \$0 \\
4--6  & Claude Sonnet 4.6 & Anthropic    & Complete & \$20 \\
7--9  & Claude Sonnet 4   & Anthropic    & Deferred & -- \\
10--12 & Claude Opus 4.6  & Anthropic    & Deferred (budget constraints) & -- \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:phase2_results} summarizes the Phase 2 results for GPT-4.1 and Claude Sonnet 4.6, each with $n=3$ independent runs from the same frozen baseline.

\begin{table}[H]
\centering
\caption{Phase 2 controlled comparison results (mean $\pm$ SD, $n=3$).}
\label{tab:phase2_results}
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{Sonnet 4.6 ($n=3$)} & \textbf{GPT-4.1 ($n=3$)} \\
\midrule
Experiments          & $112 \pm 7$               & $126 \pm 7$              \\
Kept                 & $11.3 \pm 4.9$            & $3.3 \pm 2.3$           \\
Keep rate            & $13.1\% \pm 4.9\%$        & $3.6\% \pm 2.3\%$       \\
Crash rate           & $23.5\% \pm 1.5\%$        & $28.6\% \pm 7.3\%$      \\
Best val\_bpb        & $0.9429 \pm 0.0025$       & $0.9836 \pm 0.0748$     \\
Improvement          & $15.55\% \pm 3.46\%$      & $9.81\% \pm 7.67\%$     \\
Batch halving found  & 3/3                        & 2/3                     \\
Baseline val\_bpb    & $1.1176 \pm 0.0438$       & $1.0911 \pm 0.0102$     \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:phase2_individual} shows individual run details.

\begin{table}[H]
\centering
\caption{Phase 2 individual run details.}
\label{tab:phase2_individual}
\begin{tabular}{llrrrrrr}
\toprule
\textbf{Model} & \textbf{Run} & \textbf{Exps} & \textbf{Kept} & \textbf{Keep \%} & \textbf{Crash \%} & \textbf{Baseline} & \textbf{Best val\_bpb} \\
\midrule
Sonnet 4.6 & R1 & 115 & 8  & 9.3\%  & 25.2\% & 1.0946 & 0.9439 \\
Sonnet 4.6 & R2 & 104 & 9  & 11.2\% & 23.1\% & 1.0901 & 0.9447 \\
Sonnet 4.6 & R3 & 117 & 17 & 18.7\% & 22.2\% & 1.1682 & 0.9400 \\
\midrule
GPT-4.1    & R1 & 119 & 6  & 6.3\%  & 20.2\% & 1.0966 & 0.9166 \\
GPT-4.1    & R2 & 133 & 2  & 2.2\%  & 32.3\% & 1.0793 & 1.0643 \\
GPT-4.1    & R3 & 126 & 2  & 2.4\%  & 33.3\% & 1.0974 & 0.9699 \\
\bottomrule
\end{tabular}
\end{table}
...
% -----------------------------------------------------------------------------
\subsection{Analysis}
\label{sec:phase2_analysis}
% -----------------------------------------------------------------------------

Sonnet 4.6 achieves 3.6$\times$ higher keep rate than GPT-4.1 (Cohen's $d = 2.44$, large effect). This difference is statistically meaningful even with $n=3$.

Both models reliably discover batch size halving as the primary optimization. Sonnet 4.6 found it in 3/3 runs, GPT-4.1 in 2/3. The critical difference emerges after this first discovery: Sonnet 4.6 chains additional improvements (warmdown ratio, matrix LR, gradient clipping, z-loss tuning) while GPT-4.1 gets stuck in LR-tuning loops.

GPT-4.1 shows substantially higher variance across runs (best val\_bpb SD 0.075 vs Sonnet's 0.003). R1 was an outlier with 6 improvements, while R2 and R3 found only 2 each. This unreliability is itself a finding: for autonomous overnight runs, consistency matters as much as peak performance.

Crash rates are similar (23.5\% vs 28.6\%) and not statistically different, suggesting both models struggle equally with \texttt{torch.compile} constraints. The keep rate difference is driven by proposal quality, not constraint awareness.

\paragraph{Baseline variance note.} Sonnet R3's higher baseline (1.168 vs $\sim$1.09) reflects stochastic training variance. Despite this, it achieved the best absolute val\_bpb (0.940) and highest keep rate (18.7\%), suggesting Sonnet 4.6 is robust to baseline variation.

% -----------------------------------------------------------------------------
\subsection{Phase 2 Cost}
\label{sec:phase2_cost}
% -----------------------------------------------------------------------------

Table~\ref{tab:phase2_cost} shows actual costs from billing data.

\begin{table}[H]
\centering
\caption{Phase 2 actual API costs.}
\label{tab:phase2_cost}
\begin{tabular}{lrrr}
\toprule
\textbf{Model} & \textbf{Per-experiment} & \textbf{$n=3$ total API} & \textbf{Per-run} \\
\midrule
Sonnet 4.6            & \$0.06 & $\sim$\$20 & $\sim$\$6.50 \\
GPT-4.1 (Azure)       & \$0.04 & \$0 (Azure credits) & \$0 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Actual API costs per experiment from Anthropic billing data (March 2026).}
\label{tab:budget}
\begin{tabular}{lrrr}
\toprule
...
Sonnet 4       & \$8   & \$16  & \$32  \\
Sonnet 4.6     & \$3   & \$6   & \$12  \\
Opus 4.6 (32K thinking) & \$33 & \$66  & \$132 \\
\bottomrule
\end{tabular}
\end{table}

Our recommendation: start with 100 experiments of Sonnet 4.6 (\textasciitilde\$6 API cost), then run 50--100 experiments of Opus 4.6 on the optimized result if further improvement is desired (\textasciitilde\$33--66 API cost). Sonnet 4's higher per-experiment cost (\$0.16 vs.\ \$0.06) reflects growing prompt sizes as experiment history accumulates -- the older model may also generate longer responses.
...
Phase 2 confirms the Phase 1 finding: Sonnet 4.6 achieves 3.6$\times$ higher keep rate than GPT-4.1 from the same baseline (Cohen's $d = 2.44$), with dramatically lower variance across runs. Additional future work should explore multi-GPU scaling, architecture search with more flexible compilation frameworks, evaluation on downstream tasks rather than proxy metrics, and cross-hardware validation with collaborator-contributed results.

These results suggest that LLMs are becoming practically useful for the specific subtask of iterative training optimization. Within a well-defined optimization loop with clear metrics, they can discover meaningful improvements autonomously. This study does not evaluate broader research tasks such as question formulation or scientific contextualization, and the degree to which these results generalize beyond hyperparameter tuning remains an open question.
