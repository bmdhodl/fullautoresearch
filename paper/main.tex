...
\begin{enumerate}[leftmargin=*]
    \item \textbf{First systematic multi-model comparison.} We run three state-of-the-art LLMs through sequential experimental conditions in Phase 1 and compare keep rates, crash rates, cost-effectiveness, and the quality of discovered optimizations. Phase 2 adds GPT-4.1 in a controlled head-to-head design.
...
\noindent\textit{Note: experiments where the LLM returned an unparseable response are excluded from the kept/discarded/crashed counts but included in the total. Sonnet 4 had 47 such skipped experiments, Sonnet 4.6 had 4, and Opus 4.6 had 18.}
...
We presented the first systematic comparison of LLMs as autonomous ML researchers, evaluating three models in Phase 1 on the task of optimizing a 50M-parameter language model's training configuration. Our findings support four main conclusions:
