...
% --- Abstract ---
\begin{abstract}
We evaluate four large language models as autonomous ML researchers using the \texttt{autoresearch} framework \citep{karpathy2026autoresearch}. Each model iteratively proposes modifications to a 50M-parameter language model's training code, runs 5-minute experiments on PubMed medical abstracts, and keeps or reverts changes based on validation bits-per-byte (val\_bpb). We compare Claude Sonnet 4, Claude Sonnet 4.6, Claude Opus 4.6 (with 32K extended thinking), and GPT-4.1 on identical hardware (NVIDIA RTX 5070 Ti) and dataset conditions. Sonnet 4.6 achieved the highest keep rate (22.1\%) with the lowest crash rate (8.7\%), while Opus 4.6 with extended thinking found the deepest optimizations on an already-optimized baseline, achieving the overall best val\_bpb of 0.9019. We provide detailed analysis of failure modes, cost-effectiveness, and practical guidance for researchers seeking to implement similar autonomous optimization pipelines.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{First systematic multi-model comparison.} We run four state-of-the-art LLMs through identical experimental conditions and compare keep rates, crash rates, cost-effectiveness, and the quality of discovered optimizations.
...
The Muon optimizer \citep{yang2024muon} is used for all matrix-shaped parameters, while AdamW handles embedding parameters. Flash Attention 3 \citep{dao2024flashattention3} was unavailable on the Blackwell architecture at the time of experiments, so PyTorch's scaled dot-product attention (SDPA) was used as a fallback.
...
PubMed abstracts were chosen for several reasons: the text is relatively homogeneous (structured medical writing), the dataset is large enough to avoid overfitting in 5-minute training runs, and the domain is relevant to our collaboration with St.\ Jude Children's Research Hospital.
...
Four LLM configurations were tested, summarized in Table~\ref{tab:llm_configs}.
...
\midrule
Claude Sonnet 4     & Anthropic     & None       & Adaptive (0--0.5)  & 180s  & \$0.04 \\
Claude Sonnet 4.6   & Anthropic     & None       & Adaptive (0--0.5)  & 180s  & \$0.04 \\
Claude Opus 4.6     & Anthropic     & 32K tokens & Fixed (1.0)        & 600s  & \$1--2 \\
GPT-4.1             & Azure OpenAI  & None       & Adaptive (0--0.5)  & 300s  & \$0.04 \\
\bottomrule
\end{tabular}
\end{table}

Opus 4.6 uses a fixed temperature of 1.0 as required by Anthropic's extended thinking API. The 32K thinking token budget allows the model to reason through multi-step optimization hypotheses before proposing changes. The substantially higher cost per experiment (\$1--2 vs.\ \$0.04) reflects the extended thinking token usage.
...
Opus 4.6     & 111 & 14 (16.3\%) & 64 & 26 (23.4\%) & 0.9557 & 0.9019 & 5.64 \\
GPT-4.1      & 104 & 4 (6.6\%)   & 30 & 44 (42.3\%) & 0.9636 & 0.9601 & 0.37 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Important caveat on baselines.} The four runs did not all start from the same baseline, which complicates direct comparison:
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{GPT-4.1} started from the Opus-optimized baseline baked into master (val\_bpb = 0.964).
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Batch size halving} (64 $\to$ 32): More frequent gradient updates improved convergence within the 5-minute window.
    \item \textbf{Warmdown ratio tuning} (0.5 $\to$ 0.7): Extended the cosine decay phase of the learning rate schedule.
...
Opus 4.6 with 32K extended thinking found 14 improvements in 111 experiments, but the nature of its discoveries was qualitatively different. Starting from an already well-optimized baseline (Sonnet 4.6's best), it needed to find deeper optimizations.
...
The 23.4\% crash rate was higher than Sonnet 4.6's, reflecting more aggressive experimentation. Opus 4.6 attempted structural changes that Sonnet 4.6 avoided, accepting a higher crash rate in exchange for exploring a larger optimization space.

\subsubsection{GPT-4.1}

GPT-4.1 found only 4 improvements in 104 experiments, with the highest crash rate of 42.3\%. The successful changes were minor numerical adjustments. The model struggled with two specific failure modes:

\begin{enumerate}[leftmargin=*]
    \item \textbf{\texttt{torch.compile} incompatibility.} GPT-4.1 frequently proposed changes that introduced Python constructs incompatible with \texttt{torch.compile}'s graph tracing, such as data-dependent control flow and dynamic tensor shapes.
    \item \textbf{Constraint violation.} Despite receiving experiment history showing that certain change categories consistently crashed, GPT-4.1 was slower to learn from these patterns and continued proposing similar failing modifications.
\end{enumerate}
...
\caption{Crash rates by model. Lower is better. Sonnet 4.6's low crash rate indicates the best understanding of code constraints. GPT-4.1's high crash rate reflects frequent \texttt{torch.compile} incompatible proposals.}
\label{fig:crash_rates}
\end{figure}

We categorize crashes into three types:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Compilation failures} (most common): Changes that break \texttt{torch.compile} graph tracing. These include dynamic shapes, data-dependent branches, and unsupported operations.
    ...
    \item \textbf{Inductor bugs}: Intermittent \texttt{torch.compile} backend failures unrelated to the proposed change. These are handled by auto-restart and not counted against the model.
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Crash categorization by model (approximate, based on error log analysis).}
\label{tab:crash_categories}
\begin{tabular}{lrrrr}
\toprule
\textbf{Crash Type} & \textbf{Sonnet 4} & \textbf{Sonnet 4.6} & \textbf{Opus 4.6} & \textbf{GPT-4.1} \\
\midrule
Compilation failure  & 31 (62\%) & 5 (56\%) & 14 (54\%) & 30 (68\%) \\
Runtime error        & 15 (30\%) & 3 (33\%) & 9 (35\%)  & 11 (25\%) \\
Inductor bug         & 4 (8\%)   & 1 (11\%) & 3 (11\%)  & 3 (7\%)   \\
\midrule
Total                & 50        & 9        & 26        & 44 \\
\bottomrule
\end{tabular}
...
Compilation failures dominate across all models, confirming that \texttt{torch.compile} compatibility is the primary constraint on the search space. Sonnet 4.6's low absolute crash count reflects its learned avoidance of this category after early failures.
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Metric} & \textbf{Sonnet 4} & \textbf{Sonnet 4.6} & \textbf{Opus 4.6} & \textbf{GPT-4.1} \\
\midrule
API cost (est.)      & \$5.88  & \$4.16  & \$50.00  & \$4.16  \\
GPU time (hrs)       & 12.25   & 8.67    & 9.25     & 8.67    \\
GPU cost (est.)      & \$6.13  & \$4.33  & \$4.63   & \$4.33  \\
Total cost (est.)    & \$12.01 & \$8.49  & \$54.63  & \$8.49  \\
Improvement (\%)     & 1.25    & 23.24   & 5.64     & 0.37    \\
\textbf{Cost / 1\% impr.} & \textbf{\$9.61}  & \textbf{\$0.37}  & \textbf{\$9.69}  & \textbf{\$22.95} \\
\bottomrule
\end{tabular}
\end{table}

Sonnet 4.6 is the clear efficiency winner at \$0.37 per 1\% improvement. However, this comparison is influenced by the baseline difference: Sonnet 4.6 started from the unoptimized master branch where improvements are easier to find. A fairer comparison would control for baseline difficulty, which we discuss in Section~\ref{sec:limitations}.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Constraint awareness} (crash avoidance): Sonnet 4.6 $>$ Opus 4.6 $>$ Sonnet 4 $>$ GPT-4.1
    \item \textbf{Optimization efficiency} (keep rate): Sonnet 4.6 (22.1\%) $>$ Opus 4.6 (16.3\%) $>$ GPT-4.1 (6.6\%) $>$ Sonnet 4 (5.2\%)
    \item \textbf{Depth of insight} (final val\_bpb): Opus 4.6 (0.9019) $>$ Sonnet 4 (0.9362) $>$ Sonnet 4.6 (0.9559) $>$ GPT-4.1 (0.9601)
\end{enumerate}

Constraint awareness -- the ability to learn from failed experiments and avoid repeating the same categories of errors -- appears to be the most differentiating factor for practical efficiency. Sonnet 4.6's low crash rate meant that almost every API call produced a meaningful result (either a kept improvement or an informative discard), while GPT-4.1 wasted 42\% of its compute budget on crashes.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Different baselines.} The sequential nature of our experiments -- where each model's output became the next model's baseline -- prevents apples-to-apples comparison of raw improvement percentages. A controlled experiment with identical baselines would be more rigorous.
    \item \textbf{Stochastic variation.} Each model was run once. Training stochasticity and LLM sampling randomness mean that individual experiment outcomes have variance. Multiple runs per model would strengthen statistical claims.
...
Our recommendation: start with 100 experiments of Sonnet 4.6 to establish a strong baseline (\textasciitilde\$4 API cost), then run 50--100 experiments of Opus 4.6 on the optimized result if further improvement is desired (\textasciitilde\$50--100 API cost).
...
We presented the first systematic comparison of LLMs as autonomous ML researchers, evaluating four models on the task of optimizing a 50M-parameter language model's training configuration. Our findings support four main conclusions:

\begin{enumerate}[leftmargin=*]
    \item \textbf{LLM capability directly impacts autonomous research quality.} Crash rates, keep rates, and the depth of discovered optimizations all correlate with model capability. The gap between the best (Sonnet 4.6, Opus 4.6) and worst (GPT-4.1) performers is substantial.

    \item \textbf{Sonnet 4.6 is the efficiency optimum for most researchers.} With the highest keep rate (22.1\%), lowest crash rate (8.7\%), and best cost-effectiveness (\$0.37 per 1\% improvement), Sonnet 4.6 is the recommended starting point for autonomous optimization experiments.
    ...
    \item \textbf{Open-source tooling makes this accessible.} The \texttt{autoresearch} framework, combined with consumer-grade GPUs and standard API access, enables any researcher to run autonomous optimization experiments. The total cost for a comprehensive 100-experiment run with Sonnet 4.6 is under \$10 including GPU electricity.
\end{enumerate}

Future work should address the limitations identified in Section~\ref{sec:limitations}: controlled experiments with identical baselines, multi-GPU scaling, architecture search with more flexible compilation frameworks, and evaluation on downstream tasks rather than proxy metrics.

The broader implication is that LLMs are crossing the threshold from code assistants to research collaborators. They cannot yet replace human researchers -- they lack the ability to formulate novel research questions, contextualize results in the broader scientific landscape, or reason about what experiments are worth running. But for the specific subtask of iterative training optimization, they are already practically useful.
...
We gratefully acknowledge Renato Umeton, Ph.D., and the AI division at St.\ Jude Children's Research Hospital for guidance on publication, for motivating the use of PubMed medical abstracts as the training corpus, and for championing open-source contributions to the ML research community.
...
A.~Johnson, L.~Bulgarelli, T.~Pollard, L.~Celi, R.~Mark, and S.~Horng.
\newblock {MIMIC-IV-Note}: Deidentified free-text clinical notes.
\newblock PhysioNet, 2023.
\newblock DOI: 10.13026/1n74-ne17.
...
\bibitem[Yang et~al.(2024)]{yang2024muon}
K.~Yang, A.~Wang, and others.
\newblock Muon: An optimizer for hidden layers.
\newblock 2024.
